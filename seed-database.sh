...
# This script seeds the database with sample product data
# Run this after the application has started and migrations have completed
...
if ! pg_isready -h localhost -U postgres > /dev/null 2>&1; then
    ...
    echo "Please make sure PostgreSQL is running and accessible with the default credentials."
    exit 1
fi

# Create the seed SQL
cat > seed-products.sql << 'EOF'
...
if [ -n "$DOCKER" ]; then
    ...
    PGPASSWORD=postgres psql -h postgres -U postgres -d products -f seed-products.sql
else
    # For local environment
    PGPASSWORD=postgres psql -h localhost -U postgres -d products -f seed-products.sql
fi

# Clean up
rm seed-products.sql
